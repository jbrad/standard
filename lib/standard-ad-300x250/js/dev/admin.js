...
var standard_300x250_sAdSrcId, standard_300x250_sAdSrcUrl;

standard_300x250_sAdSrcId = null;
standard_300x250_sAdSrcUrl = null;
 ...
 */
function standard_ad_300x250_hide_unused_fields($, poller) {
	"use strict";
...
} // end standard_ad_300x250_hide_unused_fields
 ...
 */
function standard_ad_300x250_show_media_uploader() {
	"use strict";
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length > 0) {
			...
			jQuery(standard_300x250_sAdSrcId).val(sImageUrl);
			jQuery(standard_300x250_sAdSrcUrl).val(jQuery(sHtml).attr('href'));

		// Otherwise, we'll just grab the image
		} else {

			sImageUrl = jQuery(sHtml).attr('src');
			jQuery(standard_300x250_sAdSrcId).val(jQuery(sHtml).attr('src'));

		} // end if

		// Display the image in the admin area
		jQuery(standard_300x250_sAdSrcId).parent()
		...
		jQuery(standard_300x250_sAdSrcId).siblings('.ad_delete')
			.show();
...
} // end standard_ad_300x250_show_media_uploader
...
function standard_ad_300x250($) {
	...
	$('.standard-ad-300x250-wrapper > .preview_image_container').unbind('click')
		.click(function(evt) {
		...
		if( '' === sId ) {
			...
			standard_300x250_sAdSrcId = '#' + $('.ad_src').attr('id');
			standard_300x250_sAdSrcUrl = '#' + $('.ad_url').attr('id');

		// Otherwise, we're running in normal mode.
		} else {

			standard_300x250_sAdSrcId = '#' + $('#' + sId + ' .widget-content .standard-ad-300x250-wrapper').children('.ad_src').attr('id');
			standard_300x250_sAdSrcUrl = '#' + $('#' + sId + ' .widget-content .standard-ad-300x250-wrapper').children('.ad_url').attr('id');

		} // end if/else

		// Show the media uploader
		standard_ad_300x250_show_media_uploader();
		$('#TB_iframeContent').load(function() {

			// if the user is uploading a new ad, we need to poll until we see the form fields
			var ad_300x250_poll = setInterval(function() {
				if($('#TB_iframeContent').contents().find('#media-items').children().length > 0) {
					standard_ad_300x250_hide_unused_fields($, ad_300x250_poll);
				} // end if
			}, 500);

			// if they aren't uplaoding, we'll clear the fields on load
			standard_ad_300x250_hide_unused_fields($);
